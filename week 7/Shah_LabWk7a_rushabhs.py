...
#Description: This program is using the 'Panda' and 'matplotlib' library to perform data analysis on the structured data- csv file and visualize the data.
#            Then, it's grouping the csv by Agency Name and Complaint Type columns using groupby method, then if finds the unique value for a column 'Agency Name', deletes unwanted columns
...
#            Even creates a nbew csv with condition on certain columns and will have selective rows and then copies the new data frame in a new
#            csv called final2.csv, finally whatever operations were performed on the readCSV frame is copied to a csv final.csv. Plots the count for each of status as well.
...
def liboperations():
    ...
    #plotting the count for each type of status using 
    df1 = readCSV['Status'].value_counts()  #gets counts for all unique values of status
    print(type(df1))
    df1.plot(kind='bar', x = 'Status', y = 'Counts', title = 'Complaint type vs counts', rot = 0)
    print('*************************************************')
    print('Now plotting the graph:')
    plt.show()

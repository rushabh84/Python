...
# Description: This is a GUI program that takes a value and calculates the assessment and propery tax value for the actual property value enetered by the user
...
class TaxGUI:
    ...
    def convert(self):
        ...
        value = 0.0
        try:
                value = float(self.prop_value.get())  #if non-float convertable value is entered in prop_value, it goes to except and by default assigns value 10000 to prop_value
                #so it handles even enter and non-float strings
                print('value is', value)
               
        except ValueError:
                print('Please enter a float value. Try again')
                self.prop_value.delete(0, END)  #deletes the content of prop_value from start index to END
                self.prop_value.insert(10, "10000")  #insert default value 10000 in the prop_value
                #self.prop_value.insert(10, input())  #incase if you want to take inout from user, can use this line of code and take value from the concolse, not needed though
